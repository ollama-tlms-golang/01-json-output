...
For this new article, we'll be using a new LLM model, **[granite3-moe:1b](https://ollama.com/library/granite3-moe:1b)**. It's slightly larger than **"baby Qwen"** (398 MB vs 822 MB) but remains a model that can run on a standard computer (without GPU) and even on a Pi5 8GB RAM. So run the following command to download it:

```bash
ollama pull granite3-moe:1b
```

**"baby Qwen"** can be "capricious" on certain complex tasks, but **"granite3-moe"** is a bit more "diligent".

> Note that **qwen2.5:0.5b** sometimes "freezes" with the 2nd JSON generation method. However, you can try with its big brother **qwen2.5:1.5b** which behaves much better.

🤔 **But what are these numbers followed by "b"?**

### LLM Parameters

The numbers followed by "b" (like 0.5b, 1b, 1.5b, 3b) represent the number of billions of parameters contained in the model. Parameters are the numerical values that the model has learned during training and that allow it to generate responses.
...
Generally, the more parameters a model has, the better it can handle complex tasks and generate sophisticated responses. It's up to you to choose the best compromise between performance and required resources according to your needs.

We are now ready to generate JSON with **"granite3-moe:1b"**.

## First Method: The Traditional Way, with a Detailed Prompt
...
```
> Note that the information generated by **"granite3-moe:1b"** generally doesn't correspond to reality. Remember, it's a baby LLM with a limited knowledge base. But that doesn't matter, we're interested in the generation mechanism.

### The Detailed Prompt

When writing the prompt, imagine you're explaining to a 5-year-old all the steps/instructions to get information about their favorite animal.
...
This code is very similar to the one in the previous article, but with a more detailed prompt.
...
Let's now move on to the second JSON generation method. Recently added to Ollama, it's simpler and more efficient (in my opinion).
...
> You can find the blog post here: [Structured outputs](https://ollama.com/blog/structured-outputs)

This new feature can be very useful for extracting document data, structuring LLM responses, and more.

Let's first see how to define the expected JSON model for our use case about animals.

### The JSON Schema

Based on Ollama's blog post example, here's how to define the JSON schema for our use case:
...
**The main difference is adding contextual data in the prompt**:
...
And when executed, you should get a response similar to this:
...
You'll notice that the model respected the JSON schema we provided and transformed text data into numerical data when necessary (for example `average_length: 1.5 to 1.75 meters` to `"average_length": 1.5`).

### Conclusion

And that's it for this article. I hope you enjoyed reading it and learned something new. This "Structured Outputs" functionality is really powerful and can be used in many use cases such as extracting data from other LLM responses, triggering actions, and more. We'll see the concept of **"tools"** in some time. But my next articles will deal with RAG (Retrieval Augmented Generation)